const bleHandler = require('./bleHandler.js');
require('babel-polyfill');

const ArgumentType = require('../../extension-support/argument-type');
const BlockType = require('../../extension-support/block-type');
const Cast = require('../../util/cast');
const log = require('../../util/log');

const USER_SERVICE_UUID = '91e4e176-d0b9-464d-9fe4-52ee3e9f1552';
const WRITE_CHARACTERISTIC_UUID = 'e9062e71-9e62-4bc6-b0d3-35cdcd9b027b';
const NOTIFY_CHARACTERISTIC_UUID = '62fbd229-6edd-4d1a-b554-5c4e1bb29169';
const _characteristicUuids = [WRITE_CHARACTERISTIC_UUID, NOTIFY_CHARACTERISTIC_UUID];
const _ble = new bleHandler.BleHandler(USER_SERVICE_UUID, _characteristicUuids);

let x = 0;
let y = 0;

/**
 * Icon svg to be displayed at the left edge of each extension block, encoded as a data URI.
 * @type {string}
 */
// eslint-disable-next-line max-len
const blockIconURI = 'data:image/svg+xml;base64,';

/**
 * Icon svg to be displayed in the category menu, encoded as a data URI.
 * @type {string}
 */
// eslint-disable-next-line max-len
const menuIconURI = 'data:image/svg+xml;base64,';

/**
 * Class for the new blocks in Scratch 3.0
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Scratch3NewBlocks {
    constructor (runtime) {
        /**
         * The runtime instantiating this block package.
         * @type {Runtime}
         */
        this.runtime = runtime;
        // this._onTargetCreated = this._onTargetCreated.bind(this);
        // this.runtime.on('targetWasCreated', this._onTargetCreated);
    }

    /**
     * @returns {object} metadata for this extension and its blocks.
     */
    getInfo () {
        return {
            id: 'myblock',
            name: 'My Block',
            menuIconURI: menuIconURI,
            blockIconURI: blockIconURI,
            blocks: [
                {
                    opcode: 'getY',
                    blockType: BlockType.REPORTER,
                    text: 'getY',
                    arguments: {
                        TEXT: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 0
                        }
                    }
                },
                {
                    opcode: 'getX',
                    blockType: BlockType.REPORTER,
                    text: 'getX',
                    arguments: {
                        TEXT: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 0
                        }
                    }
                },
                {
                    opcode: 'BLEConnect',
                    text: 'BLECoonect',
                    blockType: BlockType.REPORTER
                }
            ],
            menus: {
            }
        };
    }

    /*
     * Write log.
     * @param {object} args - the block arguments.
     * @property {number} TEXT - the text.
     */
    getX (args) {
        // const text = Cast.toString(args.TEXT);
        // log.log(text);
        return x;
    }
    
    getY (args) {
        // const text = Cast.toString(args.TEXT);
        // log.log(text);
        return y;
    }

    /**
     * Get the browser.
     * @return {number} - the user agent.
     */
    async BLEConnect () {
        if (!navigator.bluetooth) {
            log.log('BLE is not supported.');
            return;
        }
        const ret = await _ble.connect(navigator.bluetooth);

        _ble.addNotifyEvent(NOTIFY_CHARACTERISTIC_UUID, num => {
            log.log(num);
            const dataview = (new DataView(new ArrayBuffer(4)));
            dataview.setUint32(0, num, true);
            const pitch = dataview.getInt16(0, true);
            const roll = dataview.getInt16(2, true);
            log.log('pitch, roll', pitch, roll);

            x = pitch;
            y = roll;
        });
        log.log(_ble);
        return 'conected to the device : ' + _ble._serviceUuid;
    }
}


module.exports = Scratch3NewBlocks;